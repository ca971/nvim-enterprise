...
---@description CodeCompanion — AI coding companion with chat, inline assist, slash commands, multi-provider support and MCP integration
...
---
--- ╔══════════════════════════════════════════════════════════════════════════╗
--- ║  plugins/ai/codecompanion.lua — AI coding companion (multi-provider+MCP) ║
--- ║                                                                          ║
--- ║  Architecture:                                                           ║
--- ║  ┌──────────────────────────────────────────────────────────────────┐    ║
--- ║  │  Guard chain (3-level):                                          │    ║
...
--- ║  │  │  ├─ Extension: mcphub.extensions.codecompanion             │  │    ║
--- ║  │  │  │  → Tools injected as chat actions                       │  │    ║
--- ║  │  │  │  → Variables (#mcp_*) available in prompts              │  │    ║
--- ║  │  │  │  → /slash_commands from MCP servers                     │  │    ║
--- ║  │  │  │                                                         │  │    ║
--- ║  │  │  │  Available MCP tools depend on mcpservers.json:         │  │    ║
--- ║  │  │  │  ├─ filesystem → read/write/search files                │  │    ║
--- ║  │  │  │  ├─ fetch      → HTTP GET/POST any URL                  │  │    ║
--- ║  │  │  │  ├─ github     → issues, PRs, repos                     │  │    ║
--- ║  │  │  │  ├─ memory     → persistent key-value store             │  │    ║
--- ║  │  │  │  └─ (any MCP server)                                    │  │    ║
--- ║  │  │  │                                                         │  │    ║
--- ║  │  │  Without mcphub: CodeCompanion works normally (no tools)   │  │    ║
--- ║  │  │  With mcphub:    AI can call tools autonomously in chat    │  │    ║
--- ║  │  └────────────────────────────────────────────────────────────┘  │    ║
--- ║  │                                                                  │    ║
--- ║  │  Strategy routing:                                               │    ║
...
--- ║  │  ├─ MCP variables: #mcp_* from active MCP servers                │    ║
--- ║  │  ├─ MCP slash commands: /mcp_* from active MCP servers           │    ║
...
--- ║  ├─ MCP extension injected via opts.extensions — graceful degradation    ║
--- ║  │  if mcphub is not installed (extensions = {}, no error)               ║
...
--- ║  • MCP extension callback is a string (deferred require, zero cost)      ║
--- ║                                                                          ║
--- ║  Changelog:                                                              ║
--- ║  • 1.1.0 — Added MCP integration via mcphub.nvim                         ║
--- ║            mcphub added as optional dependency                           ║
--- ║            Extensions block with mcphub callback                         ║
--- ║            Cheatsheet updated with MCP variables/commands                ║
--- ╚══════════════════════════════════════════════════════════════════════════╝
...
---
---@param cmd string CodeCompanion command string
...
-- MCP INTEGRATION
--
-- Builds the extensions table for CodeCompanion's MCP support.
-- Uses a string callback (deferred require) — zero cost if mcphub
-- is not installed. CodeCompanion calls the callback lazily when
-- the chat is first opened.
--
-- Extension capabilities:
-- • show_result_in_chat:  tool call results displayed in chat
-- • make_vars:           #mcp_* variables available in prompts
-- • make_slash_commands: /mcp_* commands from active MCP servers
--
-- When mcphub is absent:
--   extensions = {}  (no error, no tools, normal operation)
-- When mcphub is present:
--   extensions.mcphub registered with callback string
-- ═══════════════════════════════════════════════════════════════════════

--- Build the MCP extensions table for CodeCompanion.
---
--- Checks if mcphub.nvim is available (installed as optional dep).
--- If available, returns an extensions table with the mcphub callback.
--- If absent, returns an empty table (graceful degradation).
---
---@return table extensions CodeCompanion extensions config
---@private
local function build_mcp_extensions()
	-- Check if mcphub module is resolvable (installed but maybe not loaded yet)
	local mcp_available = pcall(require, "mcphub")
	if not mcp_available then
		-- Also check if the plugin directory exists (lazy.nvim may not have loaded it)
		local lazy_ok, lazy_config = pcall(require, "lazy.core.config")
		if lazy_ok and lazy_config and lazy_config.plugins then
			mcp_available = lazy_config.plugins["mcphub.nvim"] ~= nil
		end
	end
...
return {
	...
	dependencies = {
		...
		-- ── MCP integration ──────────────────────────────────────
		-- Optional: when installed, MCP tools, variables, and slash
		-- commands are injected into CodeCompanion chat via extension.
		-- When absent, CodeCompanion works normally without tools.
		{
	...
	keys = {
		...
		-- ── Help ──────────────────────────────────────────────────────
		{
			...
			function()
				vim.notify(
					table.concat({
						ai_icon .. " CodeCompanion — Commands & MCP",
						string.rep("─", 50),
						"",
						"  Slash Commands:",
						...
						"",
						string.rep("─", 50),
						"  MCP (if mcphub.nvim installed):",
						"  /mcp_*       — Slash commands from MCP servers",
						"  #mcp_*       — Variables from MCP servers",
						"  Tools:       — filesystem, fetch, github, etc.",
						string.rep("─", 50),
					}, "\n"),
					vim.log.levels.INFO,
					{ title = "CodeCompanion" }
	...
	opts = {
		...
		-- ── Extensions (MCP) ─────────────────────────────────────────
		-- When mcphub.nvim is installed, registers the MCP extension
		-- which injects tools, variables, and slash commands from
		-- active MCP servers into CodeCompanion chat.
		--
		-- The callback is a string ("mcphub.extensions.codecompanion")
		-- which CodeCompanion resolves via require() lazily — zero
		-- cost at startup, tools loaded on first chat open.
		--
		-- When mcphub is absent, this resolves to {} (no extension,
		-- no error, normal operation).
		extensions = build_mcp_extensions(),
